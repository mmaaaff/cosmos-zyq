...
# NOTE: This code is copied from DanceGRPO for reference and is not a part of the COSMOS project
...
def flux_step(
    ...
    sigma = sigmas[index]
    dsigma = sigmas[index + 1] - sigma  # sigma 从 1 到 0 递减
    prev_sample_mean = latents + dsigma * model_output  # 对上一时间步的估计

    pred_original_sample = latents - sigma * model_output  # 对 x0 的估计

    delta_t = sigma - sigmas[index + 1]
    std_dev_t = eta * math.sqrt(delta_t)

    if sde_solver:
        score_estimate = -(latents-pred_original_sample*(1 - sigma))/sigma**2  # 文中 P3 倒数第二行
...
def run_sample_step(  # 多步采样，返回中间及原点预测 latent，以及 log_prob
        args,
        z,
        progress_bar,
        sigma_schedule,
    ...
    if grpo_sample:
        ...
        all_latents = torch.stack(all_latents, dim=1)  # (batch_size, num_steps + 1, 4, 64, 64), dim=1 沿时间维度 stack
        all_log_probs = torch.stack(all_log_probs, dim=1)  # (batch_size, num_steps, 1)
        return z, latents, all_latents, all_log_probs

        
def grpo_one_step(  # 进行一个去噪步，且只返回 log_prob
            args,
            latents,
            pre_latents,
            encoder_hidden_states, 
...
def train_one_step(
    ...
    #device = latents.device
    if args.use_group:  # 一个 promt 重复多遍算一组
